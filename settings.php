...
<div class="max-w-3xl mx-auto">
  ...
  <form method="post" class="mt-8 bg-white  border border-slate-200 p-6 md:p-8 space-y-5">
    <label class="block">
      ...
      <input name="arkasel_api_key" type="password" autocomplete="off" value="<?=h($values['arkasel_api_key'])?>" class="mt-1 w-full  border border-slate-200 p-3 font-mono text-sm" placeholder="Paste Arkesel API key">
      <span class="text-xs text-slate-500 mt-1 block">Used as Bearer token for standard SMS and as <code class="bg-slate-100 px-1">api-key</code> header for OTP endpoints.</span>
    </label>
    <label class="block">
      <span class="text-sm font-semibold text-slate-700">Sender ID</span>
      <input name="arkasel_sender_id" value="<?=h($values['arkasel_sender_id'])?>" class="mt-1 w-full  border border-slate-200 p-3" required>
    </label>
    <label class="block">
    ...
    <label class="block">
      ...
      <span class="text-xs text-slate-500 mt-1 block">Must accept JSON with <code class="bg-slate-100 px-1">phone_number</code>, <code class="bg-slate-100 px-1">sender_id</code>, and message containing <code class="bg-slate-100 px-1">%otp_code%</code>.</span>
    </label>
