...
<body class="bg-slate-100"><main class="min-h-screen flex items-center justify-center px-4"><div class="bg-white max-w-lg w-full  border p-8 text-center"><div class="mx-auto w-16 h-16  bg-emerald-100 text-emerald-700 flex items-center justify-center text-3xl">✓</div><h1 class="text-3xl font-black mt-6">Registration Submitted</h1><p class="text-slate-500 mt-2">Your registration has been received successfully.</p><div class="mt-6 flex justify-center gap-3"><?php if(!empty($m['pdf_path'])): ?><a target="_blank" class="px-4 py-2  bg-slate-950 text-white" href="view_pdf.php?id=<?=$id?>">View PDF</a><a class="px-4 py-2  bg-white border" href="view_pdf.php?id=<?=$id?>&download=1">Download PDF</a><?php endif; ?></div><a href="index.php" class="block mt-6 text-sm text-slate-500">Back home</a></div></main><script>(function(){try{localStorage.removeItem('mavis_registration_draft_v1');localStorage.removeItem('mavis_registration_step_v1');}catch(e){}})();</script></body></html>
