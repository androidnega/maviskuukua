...
<div class="max-w-7xl mx-auto space-y-6">
  <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
    ...
    <div class="flex flex-wrap items-center gap-2">
      ...
      <div class="flex flex-wrap gap-1.5">
        <a href="export_members_bulk.php?format=csv" class="inline-flex items-center gap-1.5 px-3 py-2  border border-slate-200 bg-white text-slate-700 text-xs font-semibold hover:bg-slate-50">Bulk CSV</a>
        <a href="export_members_bulk.php?format=excel" class="inline-flex items-center gap-1.5 px-3 py-2  border border-slate-200 bg-white text-slate-700 text-xs font-semibold hover:bg-slate-50">Bulk Excel</a>
        <a href="export_members_bulk.php?format=pdf" class="inline-flex items-center gap-1.5 px-3 py-2  border border-slate-200 bg-white text-slate-700 text-xs font-semibold hover:bg-slate-50">Bulk PDF</a>
      </div>
      <?php endif; ?>
      <?php if ($unreadCount > 0): ?>
      <span class="inline-flex items-center px-2.5 py-1  bg-emerald-50 text-emerald-800 text-xs font-semibold border border-emerald-100"><?=$unreadCount?> new to review</span>
      <?php endif; ?>
    </div>
  </div>
  ...
  <?php if ($notice): ?><div class="p-4  bg-emerald-50 text-emerald-800 border border-emerald-100 text-sm"><?=h($notice)?></div><?php endif; ?>

  <div class=" border border-slate-200 bg-slate-50/50 p-4 md:p-5">
    <form method="get" class="flex flex-col lg:flex-row lg:items-end gap-3 lg:gap-4">
      <div class="flex-1 min-w-0">
        ...
        <input type="text" name="search" value="<?=h($search)?>" placeholder="Membership ID or phone" class="w-full  border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-800 placeholder:text-slate-400">
      </div>
      <div class="w-full lg:w-52">
        <label class="block text-xs font-medium text-slate-600 mb-1.5">Sort</label>
        <select name="sort" class="w-full  border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-800">
      ...
      <button type="submit" class="w-full lg:w-auto px-5 py-2.5  bg-slate-800 text-white text-sm font-semibold hover:bg-slate-700">Apply</button>
    </form>
  </div>
            ...
            <tr class="border-t border-slate-100 hover:bg-slate-50/80">
              <td class="py-2.5 px-3 align-middle">
                <div class="flex items-center gap-2 min-w-0">
                  <?php if ($photoUrl): ?>
                    <img src="<?=h($photoUrl)?>" alt="" class="w-8 h-8 shrink-0  object-cover border border-slate-200">
                  <?php else: ?>
                    <div class="w-8 h-8 shrink-0  border border-slate-200 bg-slate-100 text-slate-400 flex items-center justify-center text-xs"><i class="fa-solid fa-user"></i></div>
                  <?php endif; ?>
                  <span class="font-semibold text-sm text-slate-800 truncate"><?=h($m['firstname'].' '.$m['surname'])?><?php if (is_new_member($m)): ?> <span class="text-[10px] px-1 py-0.5 bg-emerald-50 text-emerald-700 font-bold align-middle border border-emerald-100">NEW</span><?php endif; ?></span>
                </div>
              </td>
              ...
              <td class="py-2.5 px-3 align-middle text-right">
                ...
                <a class="shrink-0 px-2 py-1  bg-emerald-50 text-emerald-800 text-xs font-semibold border border-emerald-100" href="member.php?id=<?=$m['id']?>">Details</a>
                <?php if ($m['pdf_path']): ?>
                  <a class="shrink-0 px-2 py-1  bg-slate-50 text-slate-700 text-xs font-semibold border border-slate-200" target="_blank" href="view_pdf.php?id=<?=$m['id']?>">PDF</a>
                  <a class="shrink-0 px-2 py-1  bg-slate-50 text-slate-700 text-xs font-semibold border border-slate-200" target="_blank" href="print_pdf.php?id=<?=$m['id']?>">Print</a>
                <?php endif; ?>
                <?php if (!is_field_officer()): ?>
                <form method="post" action="regenerate_pdf.php" class="inline shrink-0">
                  <input type="hidden" name="id" value="<?=$m['id']?>">
                  <button type="submit" class="px-2 py-1  bg-indigo-50 text-indigo-800 text-xs font-semibold border border-indigo-100">Regen</button>
                </form>
                <?php endif; ?>
                ...
                <form method="post" action="delete_member.php" class="inline shrink-0" onsubmit="return confirm('Archive this member record? Files are kept for audit.');">
                  <input type="hidden" name="id" value="<?=$m['id']?>">
                  <button type="submit" class="px-2 py-1  bg-red-50 text-red-800 text-xs font-semibold border border-red-100">Delete</button>
                </form>
                <?php endif; ?>
                </div>
              </td>
  ...
  <div class="flex items-center justify-between gap-3 text-sm text-slate-600">
    ...
    <div class="flex items-center gap-2">
      <?php if ($page > 1): ?><a class="px-3 py-1.5  border border-slate-200 bg-white text-slate-700" href="<?=h(page_url($page - 1, $search, $sort))?>">Previous</a><?php endif; ?>
      <?php if ($page < $totalPages): ?><a class="px-3 py-1.5  border border-slate-200 bg-white text-slate-700" href="<?=h(page_url($page + 1, $search, $sort))?>">Next</a><?php endif; ?>
    </div>
  </div>
  <?php endif; ?>
</div>
